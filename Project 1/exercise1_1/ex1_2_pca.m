...
Data=csvread('C:\Users\nicko\MATLAB Drive\exercise1_1\data\breast_cancer_data.csv');
...
no_feature1 = 5;
no_feature2 = 10;
plot(X(Y==0, no_feature1), X(Y==0, no_feature2), 'bo',X(Y==1, no_feature1), X(Y==1, no_feature2), 'ro' );
axis square;
title('Data Samples in 2D')
xlabel('Feature 5');
ylabel('Feature 10');
...
% ADD YOUR CODE
ExplainedVar = [eigvals(1)./sum(eigvals) eigvals(2)./sum(eigvals)];
...
%  Load Face dataset
load ('C:\Users\nicko\MATLAB Drive\exercise1_1\data\faces.mat')
...
% Display reconstructed data from only k eigenfaces
subplot(1, 2, 2);
displayData(X_rec(1:100,1:K),size(X_rec(1:100,1:K),2)/10);
title('Recovered faces');
axis square;
fgr=fgr+1;
